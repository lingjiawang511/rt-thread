...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex)
{
    ...
    //
    plcddev->LCD_Fast_DrawPoint(x, y, PixelIndex);
...
{
    plcddev->LCD_Fill(x0, y0, x1, y1, LCD_COLORINDEX);
//  LCD_PIXELINDEX PixelIndex;
//  int x;
                        ...
                        const LCD_PIXELINDEX * pTrans)
{
    ...
    switch (BitsPerPixel) {
        ...
        //
        case 16:
            /* BytesPerLine is not equal to 2*(xSize), need to optimize LCD_Color_Fill */
            for (i = 0; i < ySize; i++) {
                //_DrawBitLine16BPP(pDevice, x0, i + y0, (const U16 *)pData, xSize);
                plcddev->LCD_Color_Fill(x0, i + y0, (x0 + xSize), (i + y0), (const U16 *)pData);
                pData += BytesPerLine;
            }
            break;
        //
