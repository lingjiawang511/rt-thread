...
static rt_uint8_t thread_srt20x68_handle_stack[1024];
struct rt_thread thread_srt20x68_handle;
...
static struct serial_configure srt_20x68_config = {
...
static rt_err_t uart_open(const char *name)
{
    ...
    if (uart_device != RT_NULL) {
        if (RT_EOK != rt_device_control(uart_device, RT_DEVICE_CTRL_CONFIG, (void *)&srt_20x68_config)) {
...
#define reset_srt_20x68_fsm(n) do{\
negative_flag = 0x00;\
var_srt.dew   = 0x00;\
var_srt.temp  = 0x00;\
s_checksum    = 0x00;\
...
static fsm_rt_t fsm_srt_20x68_recv(struct sensor_struct *p)
{
    ...
    switch (s_state) {
        ...
        case  RECV_NUM_V:
            ...
            if ((ch > '9') || (ch < '0')) {
                reset_srt_20x68_fsm(RECV_NUM_T);
                break;
            }
            var_srt.temp *= 10;
            var_srt.temp += ch - '0';
        ...
        case  RECV_NUM_D:
            ...
            var_srt.dew += ch - '0';
            break;
        case RECV_DATA_CHECKSUM:
            s_checksum = s_checksum & 0X003F;
            s_checksum +=  0x0020;
            if (ch == (uint8_t)s_checksum) {
                s_state = RECV_DATA_CRLF;
                p->temp =  (0x01 & negative_flag) ? -var_srt.temp : var_srt.temp;
                p->temp *= 10;
                p->dew  =  (0x02 & negative_flag) ? -var_srt.dew : var_srt.dew;
                negative_flag = 0x00;
                ...
                reset_srt_20x68_fsm(RECV_CHAR_T);
            }
            break;
        case RECV_DATA_CRLF:
            s_state = RECV_CHAR_T;
...
static void thread_entry_srt_20x68 (void* parameter)
{
    ...
    while (1) {
        if (fsm_rt_cpl == fsm_srt_20x68_recv(&srt)) {
...
int srt20x68_init(void)
{
    rt_thread_init(&thread_srt20x68_handle, "sensor_task",
                   thread_entry_srt_20x68, RT_NULL, thread_srt20x68_handle_stack,
                   sizeof(thread_srt20x68_handle_stack), 10,
                   5);
    rt_thread_startup(&thread_srt20x68_handle);
    return RT_EOK;
}

INIT_APP_EXPORT(srt20x68_init);
